...
async function getOIData(
  ...
  if (cachedData && lastScrapeTime > 0 && intervalMinutes > 0) {
    ...
    if (cacheAgeMs < maxAgeMs) {
      logger(
        `[Scraper] 🟢 Using cached data (Age: ${Math.round(cacheAgeMs / 1000)}s / Limit: ${intervalMinutes * 60}s)`,
      );
      return cachedData;
    }
  }
  ...
  try {
    if (config.appMode === "demo") {
      logger(
        `[${getTime()}] [Scraper] 🟢 DEMO MODE: Reading local dashboard file...`,
      );
      responseData = await fs.readFile(url, "utf8");
    } else if (config.appMode === "gcs") {

      logger(
        `[${getTime()}] [Scraper] ☁️ GCS MODE: Downloading dashboard file from Cloud Storage...`,
      );

      if (!config.gcsConfig.bucketName || !config.gcsConfig.dataFilename) {
        throw new Error(
          "GCS_BUCKET_NAME and GCS_DATA_FILENAME must be defined in the environment for GCS mode.",
        );
      }
      ...
      logger(
        `[${getTime()}] [Scraper] 🟠 Cache invalid or disabled. Scraping live data from URL...`,
      );
      ...
      if (proxyUrl) {
        try {
          ...
          logger(
            `[Scraper] Using Proxy Agent: ${proxyUrl.replace(/:[^:]*@/, ":***@")}`,
          );
        } catch (e) {
    ...
    osmStatusTable.find("tr").each((i, row) => {
      ...
      $(row)
        .find("td")
        .each((j, col) => cols.push($(col).text().trim()));
    ...
    if (config.appMode === "demo") {
      ...
      if (footerText) {
        ...
        if (!isNaN(referenceDate.getTime())) {
          logger(
            `[Scraper] 🗓️ Demo Reference Date found: ${formatDateToISO(referenceDate)}`,
          );
          ...
          logger(
            `[Scraper] 🔄 Adjusted ${scrapedData.length} dates relative to today.`,
          );
        } else {
          logger(
            `[Scraper] ⚠️ Found footer text but could not parse date: "${footerText}"`,
          );
        }
      } else {
        logger(
          `[Scraper] ⚠️ Could not locate reference date in static HTML. Dates remain unchanged.`,
        );
      }
    }
