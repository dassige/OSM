...
async function getUsers() { if (!db) await initDB(); return await db.all('SELECT id, email, name, role FROM users ORDER BY name ASC'); }
async function getUserById(id) { if (!db) await initDB(); return await db.get('SELECT id, email, name, role FROM users WHERE id = ?', id); }
async function getUserByEmail(email) { if (!db) await initDB(); return await db.get('SELECT id, email, name, role FROM users WHERE email = ?', email); }
async function addUser(email, name, password, role = 'simple') {
    if (!db) await initDB();
    const { salt, hash } = hashPassword(password);
    try {
        const result = await db.run(`INSERT INTO users (email, name, hash, salt, role) VALUES (?, ?, ?, ?, ?)`, email, name, hash, salt, role);
...
async function updateUser(id, name, email, role) {
    if (!db) await initDB();
    try { await db.run(`UPDATE users SET name = ?, email = ?, role = ? WHERE id = ?`, name, email, role, id); }
    catch (e) { if (e.message.includes('UNIQUE constraint')) throw new Error('Email already exists'); throw e; }
}
async function updateUserProfile(id, name, newPassword = null) {
    if (!db) await initDB();
    if (newPassword) { const { salt, hash } = hashPassword(newPassword); await db.run(`UPDATE users SET name = ?, hash = ?, salt = ? WHERE id = ?`, name, hash, salt, id); }
    else { await db.run(`UPDATE users SET name = ? WHERE id = ?`, name, id); }
}
...
// ... (Members) ...
async function getMembers() { if (!db) await initDB(); const members = await db.all('SELECT * FROM members ORDER BY name ASC'); return members.map(m => ({ ...m, enabled: m.enabled !== 0 })); }
async function addMember(member) { if (!db) await initDB(); return (await db.run(`INSERT INTO members (name, email, mobile, messengerId, enabled, notificationPreference) VALUES (?, ?, ?, ?, ?, ?)`, member.name, member.email, member.mobile, member.messengerId, member.enabled !== false ? 1 : 0, member.notificationPreference || 'email')).lastID; }
async function bulkAddMembers(members) { if (!db) await initDB(); await db.exec('BEGIN TRANSACTION'); try { const stmt = await db.prepare('INSERT INTO members (name, email, mobile, messengerId, enabled, notificationPreference) VALUES (?, ?, ?, ?, ?, ?)'); for (const member of members) { await stmt.run(member.name, member.email, member.mobile, member.messengerId, member.enabled !== false ? 1 : 0, member.notificationPreference || 'email'); } await stmt.finalize(); await db.exec('COMMIT'); } catch (error) { await db.exec('ROLLBACK'); throw error; } }
async function updateMember(id, member) { if (!db) await initDB(); await db.run(`UPDATE members SET name = ?, email = ?, mobile = ?, messengerId = ?, enabled = ?, notificationPreference = ? WHERE id = ?`, member.name, member.email, member.mobile, member.messengerId, member.enabled ? 1 : 0, member.notificationPreference || 'email', id); }
...
// ... (Skills) ...
async function getSkills() { if (!db) await initDB(); const skills = await db.all('SELECT * FROM skills ORDER BY name ASC'); return skills.map(s => ({ ...s, critical_skill: !!s.critical_skill, enabled: s.enabled !== 0, url_type: s.url_type || 'external' })); }
async function addSkill(skill) { if (!db) await initDB(); return (await db.run(`INSERT INTO skills (name, url, critical_skill, enabled, url_type) VALUES (?, ?, ?, ?, ?)`, skill.name, skill.url, skill.critical_skill ? 1 : 0, skill.enabled !== false ? 1 : 0, skill.url_type || 'external')).lastID; }
async function bulkAddSkills(skills) { if (!db) await initDB(); await db.exec('BEGIN TRANSACTION'); try { const stmt = await db.prepare('INSERT INTO skills (name, url, critical_skill, enabled, url_type) VALUES (?, ?, ?, ?, ?)'); for (const skill of skills) { await stmt.run(skill.name, skill.url, skill.critical_skill ? 1 : 0, skill.enabled !== false ? 1 : 0, skill.url_type || 'external'); } await stmt.finalize(); await db.exec('COMMIT'); } catch (error) { await db.exec('ROLLBACK'); throw error; } }
async function updateSkill(id, skill) { if (!db) await initDB(); await db.run(`UPDATE skills SET name = ?, url = ?, critical_skill = ?, enabled = ?, url_type = ? WHERE id = ?`, skill.name, skill.url, skill.critical_skill ? 1 : 0, skill.enabled ? 1 : 0, skill.url_type || 'external', id); }
...
async function getAllUserPreferences(userId) { if (!db) await initDB(); const rows = await db.all('SELECT key, value FROM user_preferences WHERE user_id = ?', userId); const prefs = {}; rows.forEach(row => { try { prefs[row.key] = JSON.parse(row.value); } catch (e) { prefs[row.key] = row.value; } }); return prefs; }
async function getUserPreference(userId, key) { if (!db) await initDB(); const row = await db.get('SELECT value FROM user_preferences WHERE user_id = ? AND key = ?', userId, key); try { return row ? JSON.parse(row.value) : null; } catch (e) { return row ? row.value : null; } }
async function saveUserPreference(userId, key, value) { if (!db) await initDB(); await db.run(`INSERT INTO user_preferences (user_id, key, value) VALUES (?, ?, ?) ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value`, userId, key, JSON.stringify(value)); }

// ... (Logs) ...
async function logEvent(user, type, title, payload) { if (!db) await initDB(); try { await db.run(`INSERT INTO event_log (user, event_type, title, payload) VALUES (?, ?, ?, ?)`, user || 'System', type, title, JSON.stringify(payload)); } catch (e) { console.error("Failed to write to event log:", e.message); } }
async function getEventLogs(filters = {}) {
    ...
    if (filters.types && filters.types.length > 0) { const placeholders = filters.types.map(() => '?').join(','); baseQuery += ` AND event_type IN (${placeholders})`; params.push(...filters.types); }
    ...
    dataQuery += ` LIMIT ? OFFSET ?`; params.push(limit, offset);
    const rows = await db.all(dataQuery, params);
    const logs = rows.map(r => { try { return { ...r, payload: JSON.parse(r.payload) }; } catch (e) { return { ...r, payload: {} }; } });
    return { logs, total, page, limit };
}
async function getEventLogMetadata() { if (!db) await initDB(); const users = await db.all('SELECT DISTINCT user FROM event_log ORDER BY user ASC'); const types = await db.all('SELECT DISTINCT event_type FROM event_log ORDER BY event_type ASC'); return { users: users.map(u => u.user), types: types.map(t => t.event_type) }; }
...
async function purgeEventLog() { if (!db) await initDB(); await db.run('DELETE FROM event_log'); }
async function pruneEventLog(days) { if (!db) await initDB(); const cutoff = new Date(); cutoff.setDate(cutoff.getDate() - days); await db.run('DELETE FROM event_log WHERE timestamp < ?', cutoff.toISOString()); }

// ... (Training & Email History) ...
async function getTrainingSessions(startDate, endDate) { if (!db) await initDB(); return await db.all('SELECT * FROM training_sessions WHERE date >= ? AND date <= ? ORDER BY date ASC', startDate, endDate); }
async function addTrainingSession(date, skillName) { if (!db) await initDB(); return (await db.run('INSERT INTO training_sessions (date, skill_name) VALUES (?, ?)', date, skillName)).lastID; }
async function deleteTrainingSession(id) { if (!db) await initDB(); await db.run('DELETE FROM training_sessions WHERE id = ?', id); }
...
async function getAllFutureTrainingSessions() { if (!db) await initDB(); const nowString = new Date().toLocaleString('en-US', { timeZone: config.timezone }); const today = new Date(nowString); const y = today.getFullYear(); const m = String(today.getMonth() + 1).padStart(2, '0'); const d = String(today.getDate()).padStart(2, '0'); return await db.all('SELECT * FROM training_sessions WHERE date >= ? ORDER BY date ASC', `${y}-${m}-${d}`); }
