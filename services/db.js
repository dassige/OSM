...
async function saveUserPreference(userId, key, value) {
    if (!db) await initDB();
    await db.run(`INSERT INTO user_preferences (user_id, key, value) VALUES (?, ?, ?) ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value`, userId, key, JSON.stringify(value));
}
async function logEvent(user, type, title, payload) {
    if (!db) await initDB();
    try { await db.run(`INSERT INTO event_log (user, event_type, title, payload) VALUES (?, ?, ?, ?)`, user || 'System', type, title, JSON.stringify(payload)); } catch (e) { console.error("Failed to write to event log:", e.message); }
}
...
// ... (Skills and other functions remain updated as per previous step) ...
async function getSkills() { if (!db) await initDB(); const skills = await db.all('SELECT * FROM skills ORDER BY name ASC'); return skills.map(s => ({ ...s, critical_skill: !!s.critical_skill, enabled: s.enabled !== 0 })); }
async function addSkill(skill) { if (!db) await initDB(); const result = await db.run(`INSERT INTO skills (name, url, critical_skill, enabled) VALUES (?, ?, ?, ?)`, skill.name, skill.url, skill.critical_skill ? 1 : 0, skill.enabled !== false ? 1 : 0); return result.lastID; }
async function bulkAddSkills(skills) { if (!db) await initDB(); await db.exec('BEGIN TRANSACTION'); try { const stmt = await db.prepare('INSERT INTO skills (name, url, critical_skill, enabled) VALUES (?, ?, ?, ?)'); for (const skill of skills) { await stmt.run(skill.name, skill.url, skill.critical_skill ? 1 : 0, skill.enabled !== false ? 1 : 0); } await stmt.finalize(); await db.exec('COMMIT'); } catch (error) { await db.exec('ROLLBACK'); throw error; } }
async function updateSkill(id, skill) { if (!db) await initDB(); await db.run(`UPDATE skills SET name = ?, url = ?, critical_skill = ?, enabled = ? WHERE id = ?`, skill.name, skill.url, skill.critical_skill ? 1 : 0, skill.enabled ? 1 : 0, id); }
...
async function pruneEventLog(days) { if (!db) await initDB(); const cutoff = new Date(); cutoff.setDate(cutoff.getDate() - days); const isoDate = cutoff.toISOString(); await db.run('DELETE FROM event_log WHERE timestamp < ?', isoDate); }
async function getEventLogsExport(filters = {}) { if (!db) await initDB(); let baseQuery = `SELECT * FROM event_log WHERE 1=1`; const params = []; if (filters.user) { baseQuery += ` AND user = ?`; params.push(filters.user); } if (filters.types && filters.types.length > 0) { const placeholders = filters.types.map(() => '?').join(','); baseQuery += ` AND event_type IN (${placeholders})`; params.push(...filters.types); } if (filters.startDate) { baseQuery += ` AND timestamp >= ?`; params.push(filters.startDate); } if (filters.endDate) { baseQuery += ` AND timestamp <= ?`; params.push(filters.endDate + ' 23:59:59'); } baseQuery += ` ORDER BY id DESC`; const rows = await db.all(baseQuery, params); return rows.map(r => { try { return { ...r, payload: JSON.parse(r.payload) }; } catch (e) { return { ...r, payload: {} }; } }); }
// Update User details
...
module.exports = {
    ...
    authenticateUser, getUserByEmail, getUsers, getUserById, addUser, updateUserProfile, adminResetPassword, deleteUser, updateUser,
