...
| T28-20 | Answering locks in and reveals correctly | Click an answer on the player's screen before time runs out. | The player's screen shows "Answer locked in — waiting..." and disables further clicks. The host screen's answered-count increases. Once the timer ends (or every player has answered), both the host and player screens automatically grey out every wrong option, leaving the correct one in its original colour. On the player's own screen only, the option they picked is additionally marked "Your answer" (even if it's one of the greyed-out wrong ones), so they can see how they did. |
