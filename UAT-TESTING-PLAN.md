...
| T22-30 | Upload category tabs | Click **Upload Document**. Observe the **Documents / Spreadsheets / Images** tabs above the Choose File button. Click **Spreadsheets**, then click **Choose File**. | The OS file picker opens filtered to Excel files. Switch back to **Documents** and reopen the picker — it now offers PDF/Word/RTF/TXT/Markdown files instead. |
| T22-31 | Upload TXT, Markdown, and image documents | Upload a `.txt` file, a `.md` file, and a `.png`/`.jpg`/`.bmp` image in turn, selecting the matching category tab first each time. | Each upload succeeds. Every new document appears in the list with the correct type badge (TXT / MD / PNG / JPG / BMP) and correct file size. |
| T22-32 | Expired document blocks public access | Edit an existing document and set its **Expiry Date** to yesterday. Click **Save**. Open its public link (`/knowledgebase/<GUID>`) in an incognito window. | The viewer shows "Document Not Available" — the same message shown for an inactive or deleted document. The file is not downloadable via the direct link either. |
| T22-33 | Extending expiry restores public access | For the document from T22-32, click **Edit** and set the **Expiry Date** to a future date. Click **Save**. Reopen the public link. | The document is viewable/downloadable again immediately — no separate re-enable step is needed. |
| T22-34 | View TXT and image documents inline | Click **View** on a `.txt` document, then on a `.png`/`.jpg`/`.bmp` document. | The TXT document's contents render directly in the page (no download needed). The image renders as a picture filling the viewer area. Both still show a working **Download** button in the header. |
...
## T28 — Live Quiz (Individual Play, Both Game Types, and Live Hosting)

**Pages:** `quiz-games.html` (Start Single), `live-quiz.html` (sessions & results), `quiz-play.html` (player access, public)

Individual play is available for both Score-based and Timed games — a session snapshots the game's questions at the moment it starts, so later edits to the game do not affect sessions already running. **Score-based** play is self-paced: answer at your own pace, submit once. **Timed** play is always live and host-driven, even solo: an admin launches a host screen from `live-quiz.html`, and every joined player answers the same question at the same time, one question at a time, with a visible countdown and a score based on speed plus correctness. Every player gets a short 6-character join code as well as a direct link — either works.

| ID | Action | Steps | Expected Result |
|----|--------|-------|----------------|
| T28-01 | Start a session, with email notification | Open a saved game (either type) with 1+ question in `quiz-games.html` → click `[Start Single]` → choose "All Active Members" or a specific selection → leave "Send email notification to invited members" ticked (default) → click `[Start]`. | Session is created. Members with an email receive an invitation email; a success toast reports how many were sent. Event log records "Quiz Session Started". |
...
| T28-03 | View sessions list | Navigate to `live-quiz.html` → **Single Sessions** tab. | List shows each session's start date, name with a Score/Timed type badge (teal for Score, orange for Timed — easy to tell apart at a glance), linked game, Active/Archived status, and "X of Y submitted" progress. |
...
| T28-14 | View a submitted player's answers | On the results view, click the "View" icon next to a member in "Submitted" status (try this for both a Score-based and a finished Timed session). | A new tab opens showing the member's answers alongside each question, with correct answers highlighted green, incorrect ones red, and any missed correct option outlined — plus the member name, score, and submission date/time. This works the same way for a Timed session once it has finished. The "View" action is not shown for members still in "Sent" status. |
...
| T28-16 | Launch button opens the right standalone window | On `live-quiz.html` → **Single Sessions**, click `[Launch]` on a Score session, then on a Timed session. | Both open a new, full-screen tab with no side menu. The Score session opens a live leaderboard; the Timed session opens a host screen with a "Waiting for players..." style lobby. `[Launch]` is not shown for an archived session. |
| T28-17 | Score leaderboard live-updates | With the Score leaderboard tab open from T28-16, have a member submit their quiz from another tab/device. | Within 10 seconds, the leaderboard tab refreshes and shows that member's rank, submitted status, and score without any manual reload. |
| T28-18 | Host screen lobby and starting the game | On the Timed host screen, confirm the roster of invited members appears, then click `[START]`. | The roster lists every invited member. After clicking Start, the host screen shows question 1 with its text, 4 coloured options, and a live countdown bar. |
| T28-19 | Players see the same question in sync | With the host on question 1, open a member's `quiz-play.html?code=...` link (private/incognito window, no login). | The player's screen shows the same question 1, the same countdown (in sync with the host), and 4 coloured answer buttons (1 Green, 2 Light Blue, 3 Red, 4 Yellow) matching the builder's convention. |
| T28-20 | Answering locks in and reveals correctly | Click an answer on the player's screen before time runs out — once for a question you'll answer correctly, once for one you'll answer wrong. | The player's screen shows "Answer locked in — waiting..." and disables further clicks. The host screen's answered-count increases. Once the timer ends (or every player has answered), both the host and player screens automatically grey out every wrong option, leaving the correct one in its original colour. On the player's own screen only, the option they picked is additionally marked "Your answer" (even if it's one of the greyed-out wrong ones), so they can see how they did — with a big tick icon in the upper-left corner of the correct option if they answered right, or a big X icon in the upper-left corner of their own picked option if they answered wrong. |
| T28-21 | Reveal → leaderboard → next question | On the host screen, after the correct answer is revealed, click the arrow button to show the leaderboard, then click `[Next Question]`. | The leaderboard ranks every player by score so far, with each player's join code shown next to their name. Clicking Next Question shows question 2 on both the host and every player's screen, with a fresh countdown. |
| T28-22 | Finishing the quiz finalizes scores | Repeat through the last question — on that final leaderboard the button reads `[Finish Quiz]` instead of `[Next Question]` — and click it. | The host screen shows a "Quiz Finished!" screen with the final ranked leaderboard. Every player's `quiz-play.html` screen independently shows its own final score screen (achieved / maximum, e.g. a 2-question quiz maxes out at 2000). Session results in `live-quiz.html` show every player as Submitted with the matching score, and each player's "View" answers page correctly marks their own picks as correct/incorrect. |
| T28-23 | Speed-based scoring still applies | Compare two players who both answer the same question correctly — one instantly, one near the deadline. | The near-instant correct answer earns close to the maximum for that question (1000 pts); the late correct answer earns roughly half (500 pts) — a wrong answer or timeout earns 0, regardless of speed. |
| T28-24 | Resuming an interrupted host screen | Mid-question or mid-reveal, close the host screen's browser tab, then click `[Launch]` again from `live-quiz.html`. | The host screen reopens showing the leaderboard for the last completed question — never mid-countdown — so the host can continue with `[Next Question]` without losing any recorded answers. |
| T28-25 | Join an individual quiz via the short code (no login) | Copy a member's join code from the session's Results view → open `/quiz` (or `quiz-join.html`) in a private/incognito window → type the code → click `[Join]`. | The code is redirected automatically to the correct quiz — a Score-based game shows its question form, a Timed game shows the live lobby/question — with no need to know or paste the full `quiz-play.html?code=...` link. |
| T28-26 | Invalid join code shows an inline error | On `quiz-join.html`, enter a made-up 6-character code that doesn't belong to any player or team → click `[Join]`. | An inline error message states the code was not found; the page does not redirect. |
| T28-27 | Score leaderboard shows join codes for pending players | Launch a Score session's leaderboard with 2+ invited members, none submitted yet. | Each still-pending row shows its member's join code in large, bold text where a status badge would otherwise be, plus a "Go to /quiz and enter your code" instruction above the list. |
| T28-28 | Join code disappears once a player submits | With the Score leaderboard open, submit that member's quiz from another window. | Within 10 seconds, that row switches to a "Submitted" badge and score — the join code is no longer shown for that row. Once everyone has submitted, the join instructions disappear entirely. |
| T28-29 | Host lobby shows join codes; joining highlights in real time | Launch a Timed session's host screen with 2+ invited members, none connected yet. | Each roster entry shows the member's name with their join code in a large font. As each member opens `/quiz` and enters their code (or opens their `quiz-play.html?code=...` link) in another window, that entry turns light green and its code disappears — without reloading the host screen. The "X of Y joined" count updates to match. |
| T28-30 | Score leaderboard offers a View button once a player submits | With the Score leaderboard open (from T28-27), submit that member's quiz from another window and wait for the leaderboard to refresh. | That row's join code is replaced by a "Submitted" badge and score, and a `[View]` button appears next to it. Clicking `[View]` opens the member's answers in a new tab, with correct answers highlighted green and incorrect ones red — identical to the `[View]` action in the Results view. Rows still pending never show a `[View]` button. |
| T28-31 | Timed leaderboard shows join codes for reconnecting | On the host or a player's screen, once a leaderboard is shown mid-game (after question 1). | Every row shows the player's join code next to their name, both on the host screen and on each player's own waiting screen — so a disconnected player, or someone resuming a paused quiz much later, can look at either screen and rejoin from `/quiz`. |
| T28-32 | Single Sessions Progress column reflects live question for Timed | Launch a Timed session and progress through a couple of questions, glancing at `live-quiz.html` → Single Sessions in another tab between steps. | Before starting: "Not started — N questions". While live: "Question X of N" matching the host screen's current question. After the last question: "Finished — N questions". A Score session in the same list still shows "X of Y submitted" unaffected. |
| T28-33 | Join/play pages force a fresh reload from the back/forward cache | Finish a quiz (individual or team) on `quiz-play.html`, then press the browser's Back button to return to `/quiz`, then Back again to `quiz-play.html`. | Both pages reload fully (a brief flash/refresh) rather than silently reappearing — `/quiz` shows an empty input ready for a new code, and `quiz-play.html` never keeps showing a stale question or the wrong quiz's result. |
| T28-34 | Player's leaderboard view hides the previous question | On the player's own screen, once the leaderboard appears between questions. | Only the leaderboard is shown — the previous question's text and answer buttons are fully gone, not left visible above or behind the leaderboard. |
| T28-35 | Player's own progress bar reflects overall quiz completion between questions | In a 6-question Timed quiz, on the player's own screen, watch the bar under "Question X of 6" while a question is live, then again once the leaderboard appears after answering question 1. | While a question is live, the bar counts down that question's time limit. Once the leaderboard appears after question 1, the bar instead shows overall progress — about 17% full (1 of 6 questions done) — rather than empty. After the final question, once "Quiz Finished" is shown, the bar is completely full (100%). |
| T28-36 | Player sees a banner when the host disconnects mid-game | Start a Timed session, join as a player, then close the host screen's browser tab (or lose its connection) while a question or leaderboard is showing. | Within a few seconds, the player's screen shows a "Host disconnected — waiting for them to reconnect..." banner. The quiz otherwise stays exactly where it was — no error, no reset. |
| T28-37 | Host-disconnected banner clears on reconnect and never shows once finished | With the banner from T28-36 showing, click `[Launch]` again from `live-quiz.html` to reopen the host screen (or open a new player after the host already reconnects). | The banner disappears within a few seconds of the host screen reopening. Reload a player's screen once the quiz is already "Quiz Finished" (host closed after finishing) — no banner appears. |
| T28-38 | Start a session without sending emails | Open a saved game (either type) with 1+ question in `quiz-games.html` → click `[Start Single]` → choose members → untick "Send email notification to invited members" → click `[Start]`. | Session is created and access codes are generated exactly as usual, but no invitation emails are sent — the toast simply confirms how many players were added ("View codes in Live Quiz"), with no mention of sending. Codes can still be read from Live Quiz and given out manually. Members' email inboxes receive nothing for this session. |

---

## T29 — Live Quiz Team Setups (Drag-and-Drop Team Building, Both Game Types, and Live Hosting)

**Pages:** `quiz-games.html` (Start Teams), `live-quiz.html` → **Team Setups** tab, `quiz-play.html` (team play, public)

Team setup is available for both Score-based and Timed games — a team session snapshots the game's questions at the moment it's set up, so later edits to the game do not affect setups already created. The whole team answers together on the captain's device via the team's access code and the result is attributed to the team: a **Score-based** team plays self-paced immediately (same flow as Single Sessions); a **Timed** team waits for an admin to launch the host screen from `live-quiz.html`, then plays one question at a time in sync with the host and every other team (same host-driven flow as an individual Timed session — see T28-16 through T28-24).
...
| T29-11 | View team setups list | Navigate to `live-quiz.html` → **Team Setups** tab. | List shows each setup's start date, name with a Score/Timed type badge (teal for Score, orange for Timed — easy to tell apart at a glance), linked game, Active/Archived status, and an "X of Y submitted" progress count. |
...
| T29-19 | Launch a Timed team setup and play (captain, no login) | Click `[Launch]` on a Timed team setup in `live-quiz.html` → **Team Setups**, click `[START]` on the host screen, then copy a team's access code and open `quiz-play.html?teamCode=...` in a private/incognito window. | The quiz loads without requiring login, labelled "Playing as Team: &lt;team name&gt;", and shows the same live question, countdown, and 4 coloured answer buttons as the host screen — in sync with every other team's device. |
...
| T29-21 | Other teams unaffected | After one team submits (Score-based), open a different team's access code from the same setup. | The other team's quiz is still answerable and unaffected by the first team's submission. |
...
| T29-24 | Multiple teams answer a live question independently | With a Timed host screen on a live question, submit different answers from two different teams' `quiz-play.html?teamCode=...` windows. | Each team's device locks in its own answer independently; the host's answered-count reflects both; the leaderboard after reveal ranks both teams by their own recorded score. |
| T29-25 | Join a team quiz via the short code (captain, no login) | Copy a team's join code from the Teams view (`[Copy Code]`) → open `/quiz` (or `quiz-join.html`) in a private/incognito window → type the code → click `[Join]`. | The code is redirected automatically to the correct team quiz, labelled "Playing as Team: &lt;team name&gt;" — with no need to know or paste the full `quiz-play.html?teamCode=...` link. |
| T29-26 | Score leaderboard shows join codes for pending teams | Launch a Score team setup's leaderboard with 2+ teams, none submitted yet. | Each still-pending team's row shows its join code in large, bold text in place of a status badge, plus a "Go to /quiz and enter your code" instruction above the list — same behaviour as individual sessions. |
| T29-27 | Score leaderboard offers a View button once a team submits | With the Score team leaderboard open (from T29-26), submit that team's quiz from another window and wait for the leaderboard to refresh. | That team's row is replaced by a "Submitted" badge and score, and a `[View]` button appears next to it. Clicking `[View]` opens the team's answers in a new tab, correct/incorrect marked — identical to the `[View]` action in the Teams view. |
| T29-28 | Team Setups Progress column reflects live question for Timed | Launch a Timed team setup and progress through a couple of questions, glancing at `live-quiz.html` → Team Setups in another tab between steps. | Before starting: "Not started — N questions". While live: "Question X of N" matching the host screen's current question. After the last question: "Finished — N questions". A Score team setup in the same list still shows "X of Y submitted" unaffected. |
