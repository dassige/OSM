...
function parseRankAndName(rowOrName) {
  if (rowOrName && typeof rowOrName === "object") {
    ...
    let displayName;
    if (rowOrName.member_last_name) {
      displayName = window.formatMemberName
        ? window.formatMemberName(null, rowOrName.member_last_name, rowOrName.member_first_name, rowOrName.member_name)
        : rowOrName.member_last_name;
    } else {
      displayName = rowOrName.member_name || "";
    }
