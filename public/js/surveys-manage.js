    ...
    fetch("/ui-config")
        ...
        .then((c) => {
            ...
            if (c.loginTitle) {
                ...
                document.getElementById("pageHeader").innerText = "Surveys Manager - " + c.loginTitle;
            }
...
function getSurveyData() {
    ...
    const intro = tinymce.get("surveyIntro") ? tinymce.get("surveyIntro").getContent() : "";
...
async function saveSurvey() {
    ...
    const url = currentSurvey && currentSurvey.id ? `/api/surveys/${currentSurvey.id}` : "/api/surveys";
...
async function deleteSurvey() {
    ...
    try {
        if (!(await confirmAction("Delete Survey", `Are you sure you want to delete '${currentSurvey.name}'? This action cannot be undone.`))) return;

        const delRes = await fetch(`/api/surveys/${currentSurvey.id}`, { method: "DELETE" });
...
async function previewSurvey() {
    ...
    window.open(`surveys-view.html?id=${currentSurvey.public_id}&preview=true`, "_blank");
}
...
function importSingleSurvey(input) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (!data.name || !Array.isArray(data.structure)) throw new Error("Invalid format");
            ...
            showToast("Survey imported into editor. Click Save to persist.", "success");
        } catch (err) {
...
async function importAllSurveys(input) {
    ...
    if(uiConfig?.appMode === 'demo') return showToast("Bulk import disabled in Demo Mode", "warning");

    if (!(await confirmAction("Bulk Import", "WARNING: This will DELETE ALL existing surveys and replace them with the imported file.\n\nAre you sure?"))) {
        input.value = "";
        return;
    }
    ...
    try {
        const res = await fetch("/api/surveys/import/all", { method: "POST", body: formData });
...
function renderSurveyList() {
    ...
    const sortedSurveys = [...surveys].sort((a, b) => {
        ...
        switch (surveySortMode) {
            ...
            case "status_active": return statusA !== statusB ? statusB - statusA : nameA.localeCompare(nameB);
            case "status_disabled": return statusA !== statusB ? statusA - statusB : nameA.localeCompare(nameB);
            default: return 0;
        }
    });
...
// Enable Publish button only if survey is saved and Active (status === 1)
    ...
    setTimeout(() => { originalSurveyState = getSurveyData(); }, 500);
}
...
function renderFieldItem(field) {
    ...
    if (field.type === "radio" || field.type === "checkboxes" || field.type === "boolean") {
...
window.toggleAllFields = function () {
    ...
    cards.forEach((c) => anyCollapsed ? c.classList.add("expanded") : c.classList.remove("expanded"));
