...
async function runStat() {
  ...
  try {
    ...
    statPanel.innerHTML = `<p style="color:red;">Error: ${e.message}</p>`;
  }
}
...
async function downloadPdf() {
  ...
  if (!contentPanel || contentPanel.innerHTML.includes("Refresh Data")) {
    if (window.showToast)
      showToast("Please run a statistic view first.", "warning");
    return;
  }
  ...
  const chartImages = Array.from(charts).map((canvas) =>
