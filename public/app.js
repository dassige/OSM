...
function updateSendButtonState() {
  ...
  const anyChecked =
    document.querySelectorAll(".send-email-cb:checked, .send-wa-cb:checked")
      .length > 0;
...
function applySort() {
  ...
  if (iconSpan) {
    iconSpan.textContent = currentSort.order === "asc" ? ICON_ASC : ICON_DESC;
...
function renderTable() {
  ...
  currentOsmData.forEach((member, index) => {
    ...
    if (hasVisibleSkills) {
      ...
      skillTd.textContent = buildSkillHtml(visibleSkills[0], member.id);
    ...
    if (member.emailEligible && hasVisibleSkills) {
      ...
      emailLabel.textContent = `<input type="checkbox" class="send-email-cb" data-name="${
        member.name
      ...
      btnEmail.textContent = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>`;
      ...
      waLabel.textContent = `<input type="checkbox" class="send-wa-cb" data-name="${
        member.name
      }" ${
    ...
    for (let i = 1; i < visibleSkills.length; i++) {
      ...
      subSkillTd.textContent = buildSkillHtml(visibleSkills[i], member.id);
