...
router.get('/instances/:liveId/tracking', hasRole('admin'), async (req, res) => {
    try {
        ...
        if (!trackingData) return res.status(404).json({ error: 'No tracking data found.' });
...
router.post('/', hasRole('admin'), async (req, res) => {
    try {
        ...
        const result = await db.createSurvey(name, intro, status || 0, JSON.stringify(structure), authorId);
...
router.get('/instances/:liveId/results', hasRole('admin'), async (req, res) => {
    try {
        ...
        if (!rawData || !rawData.instance) return res.status(404).json({ error: 'Survey instance not found.' });
        ...
        const aggregatedData = structure.map(question => {
            ...
            if (question.type === 'radio' || question.type === 'checkboxes') {
                ...
                result.data['Yes'] = 0; result.data['No'] = 0;
            }
            ...
            parsedResponses.forEach(ans => {
                ...
                if (question.type === 'text_multi') {
                    ...
                    val.forEach(v => { if (result.data[v] !== undefined) result.data[v]++; });
                } else {
        ...
        res.json({
            survey: { id: rawData.instance.id, name: rawData.instance.name, publishedAt: rawData.instance.published_at },
            stats: {
                totalInvited: rawData.trackingStats.totalInvited || 0,
                totalSubmitted: rawData.trackingStats.totalSubmitted || 0,
                completionRate: rawData.trackingStats.totalInvited > 0 ? Math.round((rawData.trackingStats.totalSubmitted / rawData.trackingStats.totalInvited) * 100) : 0
            },
...
router.put('/:id', hasRole('admin'), async (req, res) => {
    try {
        ...
        if (!name || !structure) return res.status(400).json({ error: 'Survey name and structure are required.' });
...
router.post('/:id/publish', hasRole('admin'), async (req, res) => {
    try {
        ...
        const { liveInstanceId, trackingData } = await db.publishSurvey(surveyId, memberIds, authorId);
        ...
        res.json({ message: `Survey published successfully! Links generated for ${trackingData.length} members.` });
    } catch (error) {
...
router.post('/instances/:liveId/remind-all', hasRole('admin'), async (req, res) => {
    try {
        const liveId = req.params.liveId;
        ...
        res.json({ message: `Reminder emails triggered for ${pending.length} pending members.` });
    } catch (error) {
...
router.post('/instances/:liveId/remind/:trackingId', hasRole('admin'), async (req, res) => {
    try {
        ...
        if (record.status !== 'pending') return res.status(400).json({ error: 'Member has already submitted the survey.' });

        // --- EMAIL DISPATCH (Placeholder) ---
        // Example hook:
        // await mailer.sendSurveyReminderEmail(record.email, record.access_code);

        res.json({ message: `Reminder email triggered for ${record.member_name}.` });
    } catch (error) {
